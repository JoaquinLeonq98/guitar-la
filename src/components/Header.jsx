...
export default function Header({cart, removeFromCart, addQuantity, removeQuantity, clearCart}) {

    // state Derivado
    const emptyCart = useMemo(() => cart.length === 0, [cart]);
    const totalCart = useMemo(() => cart.reduce((total, item) => total + (item.price * item.quantity),0));
